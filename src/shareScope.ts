...
function ensureScope(): any {
  if (!sharedScope) {
    throw new Error("[ai-sdk-chat-kernel] Shared scope has not been initialised");
  }
  return sharedScope;
}
...
export function getSharedModule<T = any>(moduleName: string): T {
  ...
  if (!versions) {
    throw new Error(`[ai-sdk-chat-kernel] Module ${moduleName} not found in shared scope`);
  }
  const versionKeys = Object.keys(versions);
  if (versionKeys.length === 0) {
    throw new Error(`[ai-sdk-chat-kernel] No versions available for ${moduleName}`);
  }
  const selected = versions[versionKeys[0]];
  const factory = selected?.get;
  if (typeof factory !== "function") {
    throw new Error(`[ai-sdk-chat-kernel] Shared module ${moduleName} missing factory`);
  }
  const moduleExports = factory();
  if (moduleExports && typeof (moduleExports as Promise<any>).then === "function") {
    throw new Error("[ai-sdk-chat-kernel] Async shared modules are not supported");
  }
